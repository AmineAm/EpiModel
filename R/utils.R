...
#' }
#'
brewer_ramp.bip <- function(n, plt, delete.lights = TRUE){
...
#' @export
#' @keywords internal
deleteAttr.bip <- function(attrList, ids) {
...
#'      vertex.cex = 1.5, edge.col = "grey50")
#'
transco.bip <- function(col, alpha = 1, invisible = FALSE) {
...
#' @export
#' @keywords internal
ssample.bip <- function(x, size, replace = FALSE, prob = NULL) {
...
#' @title Add New Epidemiology Variables
#'
#' @description Inspired by \code{dplyr::mutate}, \code{mutate_epi.bip} adds new
#'              variables to the epidemiological and related variables within
#'              simulated model objects of any class in \code{EpiModel}.
#'
...
#' @examples
#' # DCM example
#' param <- param.dcm.bip(inf.prob = 0.2, act.rate = 0.25)
#' init <- init.dcm.bip(s.num = 500, i.num = 1)
#' control <- control.dcm.bip(type = "SI", nsteps = 500)
#' mod1 <- dcm.bip(param, init, control)
#' mod1 <- mutate_epi(mod1, prev = i.num/num)
#' plot(mod1, y = "prev")
#'
#' # Network model example
...
#' init <- init.net.bip(i.num = 1, i.num.m2 = 0)
