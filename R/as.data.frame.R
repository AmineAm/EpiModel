...
#' @title Extract Model Data for Deterministic Bipartite Compartmental Models
#'
#' @description This function extracts a model run from an object of class
#'              \code{dcm} into a data frame using the generic
#'              \code{as.data.frame} function.
...
#'
#' @examples
#' ## Example 1: Two-group SIR model with vital dynamics
#' param <- param.dcm(inf.prob = 0.2, inf.prob.g2 = 0.1,
#'                    act.rate = 3, balance = "g1",
#'                    rec.rate = 1/50, rec.rate.g2 = 1/50,
#'                    a.rate = 1/100, a.rate.g2 = NA,
...
#' as.data.frame(mod2)
#'
as.data.frame.dcm.bip <- function(x, row.names = NULL, optional = FALSE, run,
                                  ...) {
...
#' @title Extract Model Data for Bipartite Stochastic Models
#'
#' @description This function extracts model simulations for objects of classes
#'              \code{icm} and \code{netsim} into a data frame using
#'              the generic \code{as.data.frame} function.
...
#' @examples
#' ## Stochastic ICM SIS model
#' param <- param.icm(inf.prob = 0.8, act.rate = 2, rec.rate = 0.1) (Note: bip example)
#' init <- init.icm(s.num = 500, i.num = 1)
#' control <- control.icm(type = "SIS", nsteps = 10,
#'                        nsims = 3, verbose = FALSE)
#' mod <- icm(param, init, control)
...
#' }
#'
as.data.frame.icm.bip <- function(x, row.names = NULL, optional = FALSE,
                                  out = "vals", sim, qval, ...) {
...
#' @export
#' @rdname as.data.frame.icm
as.data.frame.netsim.bip <- function(x, row.names = NULL, optional = FALSE,
                                     out = "vals", sim, ...) {
  if (missing(sim)) {
    sim <- 1:x$control$nsims
  }
...
#' @examples
#' # Initialize and parameterize the network model
#' nw <- network.initialize(n = 100, directed = FALSE) (Note: bip example)
#' formation <- ~edges
#' target.stats <- 50
#' coef.diss <- dissolution_coefs(dissolution = ~offset(edges), duration = 20)
#'
...
#' as.data.frame(dx)
#'
as.data.frame.netdx.bip <- function(x, row.names = NULL, optional = FALSE,
                                    sim, ...) {
