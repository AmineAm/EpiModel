...
init_status.net <- function(dat) {
  ...
  if (statOnNw == TRUE) {
    ...
    if (!is.null(status.vector)) {
      ...
      status <- rep("s", num)
      status[sample(1:num, size = i.num)] <- "i"
      if (type == "SIR") {
        status[sample(which(status == "s"), size = r.num)] <- "r"
      }
    }
  }
...
#' @title Initialization: bipartite netsim Module
#'
#' @description This function initializes the master \code{dat} object on which
#'              data are stored, simulates the initial state of the network, and
#'              simulates disease status and other attributes.
...
#' @param param An \code{EpiModel} object of class \code{\link{param.net.bip}}.
#' @param init An \code{EpiModel} object of class \code{\link{init.net.bip}}.
#' @param control An \code{EpiModel} object of class \code{\link{control.net.bip}}.
#' @param s Simulation number, used for restarting dependent simulations.
#'
#' @export
#' @keywords internal
#'
initialize.net.bip <- function(x, param, init, control, s) {
...
#' time loop starts.
#'
#' @seealso This is an initialization module for \code{\link{netsim.bip}}.
#'
#' @export
#' @keywords netMod internal
#'
init_status.net.bip <- function(dat) {
...
#'              a \code{networkDynamic.bip} object.
#'
#' @param nw An object of class \code{networkDynamic.bip}.
#' @param prefixes Character string prefix for mode-specific ID.
#'
#' @details
#' This function is used for \code{\link{netsim.bip}} simulations
...
#' \code{\link{arrivals.net.bip}} module when the persistent IDs of incoming nodes
#' must be set.
#'
#' @export
#' @keywords netMod internal
#' @seealso \code{\link{initialize.pids.bip}}
#'
#' @examples
#' # Initialize network with 25 female and 75 male
#' nw <- network.initialize.bip(100, bipartite = 25)
#'
...
#' nw <- init_pids.bip(nw, c("A", "B"))
#' nw %v% "vertex.names"
#'
init_pids.bip <- function(nw, prefixes=c("F", "M")) {
