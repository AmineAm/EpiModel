...
#'              dynamic/temporal models (STERGM).
#'
#' @param nw An object of class \code{network.bip}.
#' @param formation Right-hand sided STERGM formation formula in the form
#'        \code{~edges + ...}, where \code{...} are additional network statistics.
#' @param target.stats Vector of target statistics for the formation model, with
#'        one number for each network statistic in the model.
#' @param coef.diss An object of class \code{disscoef.bip} output from the
#'        \code{\link{dissolution_coefs.bip}} function.
#' @param constraints Right-hand sided formula specifying constraints for the
#'        modeled network, in the form \code{~...}, where \code{...} are constraint
#'        terms. By default, no constraints are set.
#' @param coef.form Vector of coefficients for the offset terms in the formation
...
#'
#' @details
#' \code{netest.bip} is a wrapper function for the \code{ergm} and \code{stergm}
#' functions that estimate static and dynamic network models, respectively.
#' Network model estimation is the first step in simulating a stochastic network
#' epidemic model in \code{EpiModel}. The output from \code{netest.bip} is a
#' necessary input for running the epidemic simulations in \code{\link{netsim.bip}}.
#' With a fitted network model, one should always first proceed to model
#' diagnostics, available through the \code{\link{netdx.bip}} function, to check
#' model fit. A detailed description of fitting these models, along with examples,
#' may be found in the
#' \href{http://statnet.github.io/tut/BasicNet.html}{Basic Network Models}
#' tutorial.
...
#' model fitting process. When fitting a STERGM directly (setting \code{edapprox}
#' to \code{FALSE}), control parameters may be passed to the \code{stergm}
#' function with the \code{set.control.stergm} argument in \code{netest.bip}. The
#' controls should be input through the \code{control.stergm()} function, with
#' the available parameters listed in the \code{\link{control.stergm}} help page
#' in the \code{tergm} package.
#'
#' When fitting a STERGM indirectly (setting \code{edapprox} to \code{TRUE}),
#' control settings may be passed to the \code{ergm} function using
#' \code{set.control.ergm} in \code{netest.bip}. The controls should be input through
#' the \code{control.ergm()} function, with the available parameters listed in the
#' \code{\link[ergm:control.ergm]{control.ergm}} help page in the \code{ergm}
#' package. An example is below.
#'
...
#'
#' @keywords model
#' @seealso Use \code{\link{netdx.bip}} to diagnose the fitted network model, and
#'          \code{\link{netsim.bip}} to simulate epidemic spread over a simulated
#'          dynamic network consistent with the model fit.
#'
#' @export
#'
...
#'
#' # Obtain the offset coefficients
#' coef.diss <- dissolution_coefs.bip(dissolution = ~offset(edges), duration = 10)
#'
#' # Estimate the STERGM using the edges dissolution approximation
...
#'
#' # To estimate the STERGM directly, use edapprox = FALSE
#' # est2 <- netest.bip(nw, formation, target.stats, coef.diss, edapprox = FALSE)
#'
netest.bip <- function(nw, formation, target.stats, coef.diss, constraints,
                       coef.form = NULL, edapprox = TRUE, output = "fit",
                       set.control.ergm, set.control.stergm,
                       verbose = FALSE) {
  ...
  if (class(coef.diss) != "disscoef.bip") {
    stop("dissolution must be of input through dissolution_coefs.bip function",
         call. = FALSE)
  }
  dissolution <- coef.diss$dissolution
  diss_check(formation, dissolution)
  ...
  class(out) <- "netest.bip"
  return(out)
}


diss_check.bip <- function(formation, dissolution){
...
#'
#' @description Adjusts the dissolution component of an dynamic ERGM fit using
#'              the \code{netest.bip} function with the edges dissolution approximation
#'              method.
#'
#' @param old.netest An object of class \code{netest.bip}, from the \code{\link{netest.bip}}
#'        function.
#' @param new.coef.diss An object of class \code{disscoef.bip}, from the
#'        \code{\link{dissolution_coefs.bip}} function.
#'
#' @details
#' Fitting an ERGM is a computationally intensive process when the model includes
...
#' @examples
#' \dontrun{
#' #(Need Two Example Case)
#' nw <- network.initialize.bip(1000, directed = FALSE)
#'
#' # Two dissolutions: an average duration of 300 versus 200
#' diss.300 <- dissolution_coefs.bip(~offset(edges), 300, 0.001)
#' diss.200 <- dissolution_coefs.bip(~offset(edges), 200, 0.001)
#'
#' # Fit the two reference models
#' est300 <- netest.bip(nw = nw,
#'                 formation = ~edges,
#'                 target.stats = c(500),
#'                 coef.diss = diss.300)
#'
#' est200 <- netest.bip(nw = nw,
#'                 formation = ~edges,
#'                 target.stats = c(500),
#'                 coef.diss = diss.200)
#'
#' # Alternatively, update the 300 model with the 200 coefficients
#' est200.compare <- update_dissolution.bip(est300, diss.200)
#'
#' identical(est200$coef.form, est200.compare$coef.form)
#'}
#'
update_dissolution.bip <- function(old.netest, new.coef.diss) {
