...
#' @title Summary Model Statistics
#'
#' @description Extracts and prints model statistics solved with \code{dcm.bip}.
#'
#' @param object An \code{EpiModel} object of class \code{dcm.bip}.
#' @param at Time step for model statistics.
#' @param run Model run number, for \code{dcm.bip} class models with multiple runs
#'        (sensitivity analyses).
#' @param digits Number of significant digits to print.
#' @param ... Additional summary function arguments (not used).
#'
#' @details
#' Summary statistics for the main epidemiological outcomes (state and transition
#' size and prevalence) from an \code{dcm.bip} model. Time-specific summary measures
#' are provided, so it is necessary to input a time of interest. For multiple-run
#' models (sensitivity analyses), input a model run number. See examples below.
#'
#' @seealso \code{\link{dcm.bip}}
#'
#' @method summary dcm
#' @keywords extract
#' @export
#'
#' @examples
#' ## Deterministic Bipartite SIR model with varying act.rate (Example)
#' param <- param.dcm.bip(inf.prob = 0.2, act.rate = 2:4, rec.rate = 1/3,
#'                    a.rate = 0.011, ds.rate = 0.01,
#'                    di.rate = 0.03, dr.rate = 0.01)
#' init <- init.dcm.bip(s.num = 1000, i.num = 1, r.num = 0)
#' control <- control.dcm.bip(type = "SIR", nsteps = 50)
#' mod <- dcm.bip(param, init, control)
#' summary(mod, at = 25, run = 1)
#' summary(mod, at = 25, run = 3)
#' summary(mod, at = 26, run = 3)
#'
summary.dcm.bip <- function(object, at, run = 1, digits = 3, ...) {
  ...
  if (!is.null(object$control$new.mod)) {
    stop("summary method not available for new model types in dcm.bip")
  }
...
#' @title Summary Model Statistics
#'
#' @description Extracts and prints model statistics simulated with \code{icm.bip}.
#'
#' @param object An \code{EpiModel} object of class \code{icm.bip}.
#' @param at Time step for model statistics.
#' @param digits Number of significant digits to print.
#' @param ... Additional summary function arguments.
#'
#' @details
#' Summary statistics for the main epidemiological outcomes (state and transition
#' size and prevalence) from an \code{icm.bip} model. Time-specific summary measures
#' are provided, so it is necessary to input a time of interest.
#'
...
#'
#' @examples
#' ## Stochastic ICM SI model with 3 simulations (Need Example)
#' param <- param.icm.bip(inf.prob = 0.2, act.rate = 1)
#' init <- init.icm.bip(s.num = 500, i.num = 1)
#' control <- control.icm.bip(type = "SI", nsteps = 50,
#'                        nsims = 5, verbose = FALSE)
#' mod <- icm.bip(param, init, control)
#' summary(mod, at = 25)
#' summary(mod, at = 50)
#'
summary.icm.bip <- function(object, at, digits = 3, ...) {
...
#' @title Summary Model Statistics
#'
#' @description Extracts and prints model statistics simulated with \code{netsim.bip}.
#'
#' @param object An \code{EpiModel} object of class \code{netsim.bip}.
#' @param at Time step for model statistics.
#' @param digits Number of significant digits to print.
#' @param ... Additional summary function arguments.
#'
#' @details
#' Summary statistics for the main epidemiological outcomes (state and transition
#' size and prevalence) from a \code{netsim.bip} model. Time-specific summary
#' measures are provided, so it is necessary to input a time of interest.
#'
#' @seealso \code{\link{netsim.bip}}
#'
#' @method summary netsim
#' @keywords extract
#' @export
...
#' control <- control.net.bip(type = "SI", nsteps = 100, nsims = 5, verbose.int = 0)
#'
#' # Run the model simulation
#' mod <- netsim.bip(est1, param, init, control)
#'
#' summary(mod, at = 1)
#' summary(mod, at = 50)
#' summary(mod, at = 100)
#' }
#'
summary.netsim.bip <- function(object, at, digits = 3, ...) {
...
#' @description Prints the summary model fit statistics for an ERGM or STERGM fit.
#'
#' @param object An \code{EpiModel} object of class \code{netest.bip}.
#' @param ... Additional summary function arguments.
#'
#' @method summary netest
#' @keywords internal
...
#' will be printed.
#'
summary.netest.bip <- function(object, ...) {
