...
init_status.icm <- function(dat) {
  ...
  # Status ------------------------------------------------------------------
  status <- rep("s", nG1)
  status[sample(1:nG1, size = i.num)] <- "i"
...
#' @title Initialization: bipartite icm Module
#'
#' @description This function initializes the master \code{dat} object on which
#'              data are stored, and simulates disease status and other attributes.
#'
...
#' @keywords internal
#'
initialize.icm.bip <- function(param, init, control) {
...
#' @title Disease Status Initialization Module for bipartite icm
#'
#' @description This function sets the initial disease status on the
#'              network given the specified initial conditions.
#'
#' @param dat Master data list object.
#'
#' @seealso This is an initialization module for \code{\link{icm.bip}}.
#'
#' @export
#' @keywords internal
#'
init_status.icm.bip <- function(dat) {
