...
#' @title Merge Data across Stochastic Bipartite Individual Contact Model Simulations
#'
#' @description Merges epidemiological data from two independent simulations of
#'              stochastic bipartite individual contact models from \code{icm.bip}.
#'
#' @param x An \code{EpiModel} object of class \code{\link{icm.bip}}.
#' @param y Another \code{EpiModel} object of class \code{\link{icm.bip}}, with the
#'        identical model parameterization as \code{x}.
#' @param ...  Additional merge arguments (not used).
#'
#' @details
#' This merge function combines the results of two independent simulations of
#' \code{\link{icm.bip}} class models, simulated under separate function calls. The
#' model parameterization between the two calls must be exactly the same, except
#' for the number of simulations in each call. This allows for manual
#' parallelization of model simulations.
#'
...
#'
#' @examples
#' param <- param.icm.bip(inf.prob = 0.2, act.rate = 0.8)
#' init <- init.icm.bip(s.num = 1000, i.num = 100)
#' control <- control.icm.bip(type = "SI", nsteps = 10,
#'                        nsims = 3, verbose = FALSE)
#' x <- icm.bip(param, init, control)
#'
#' control <- control.icm.bip(type = "SI", nsteps = 10,
#'                        nsims = 1, verbose = FALSE)
#' y <- icm.bip(param, init, control)
#'
#' z <- merge(x, y)
#' x$epi
#' y$epi
...
#' @title Merge Model Simulations Across netsim.bip Objects
#'
#' @description Merges epidemiological data from two independent simulations of
#'              stochastic bipartite network models from \code{netsim.bip}.
#'
#' @param x An \code{EpiModel} object of class \code{\link{netsim.bip}}.
#' @param y Another \code{EpiModel} object of class \code{\link{netsim.bip}},
#'        with the identical model parameterization as \code{x}.
#' @param keep.transmat If \code{TRUE}, keep the transmission matrices from the
#'        original \code{x} and \code{y} elements.
#' @param keep.network If \code{TRUE}, keep the \code{networkDynamic.bip} objects
#'        from the original \code{x} and \code{y} elements.
#' @param keep.nwstats If \code{TRUE}, keep the network statistics (as set by
#'        the \code{nwstats.formula} parameter in \code{control.netsim.bip}) from
#'        the original \code{x} and \code{y} elements.
#' @param keep.other If \code{TRUE}, keep the other simulation elements (as set
#'        by the \code{save.other} parameter in \code{control.netsim.bip}) from the
#'        original \code{x} and \code{y} elements.
#' @param param.error If \code{TRUE}, if \code{x} and \code{y} have different
#'        params (in \code{param.net.bip}) or controls (passed in \code{control.net.bip})
#'        an error will prevent the merge. Use \code{FALSE} to override that
#'        check.
#' @param ...  Additional merge arguments (not currently used).
#'
#' @details
#' This merge function combines the results of two independent simulations of
#' \code{\link{netsim.bip}} class models, simulated under separate function calls. The
#' model parameterization between the two calls must be exactly the same, except
#' for the number of simulations in each call. This allows for manual
#' parallelization of model simulations.
#'
...
#'
#' # Epidemic models
#' param <- param.net.bip(inf.prob = 1)
#' init <- init.net.bip(i.num = 1)
#' control <- control.net.bip(type = "SI", nsteps = 20, nsims = 2,
#'                        save.nwstats = TRUE,
#'                        nwstats.formula = ~edges + degree(0),
#'                        verbose = FALSE)
#' x <- netsim.bip(est, param, init, control)
#' y <- netsim.bipest, param, init, control)
#'
#' # Merging
#' z <- merge(x, y)
#' x$epi
...
#' z$epi
#'
merge.netsim.bip <- function(x, y, keep.transmat = TRUE, keep.network = TRUE,
                             keep.nwstats = TRUE, keep.other = TRUE,
                             param.error = TRUE, ...) {
