...
#' @title Convert transmat infection tree into a phylo object
#'
#' @description Converts the edgelist matrix in the \code{transmat.bip} object into a
#'              \code{phylo.bip} object by doing the required reordering and labeling.
#'
#' @param x An object of class \code{"transmat.bip"}, the output from
#'        \code{\link{get_transmat.bip}}.
#' @param collapse.singles logical, DEPRECATED
#' @param vertex.exit.times  optional numeric vector providing the time of departure
#'        of vertices, to be used to scale the lengths of branches reaching to
#'        the tips. Index position on vector corresponds to network id. NA indicates
...
#'
#' @details
#' Converts a \code{\link{transmat.bip}} object containing information about the history
#' of a simulated infection into a \code{\link{phylo.bip}} object representation
#' suitable for plotting as a tree with \code{\link[ape]{plot.phylo}}. Each
#' infection event becomes a 'node' (horizontal branch) in the resulting phylo tree,
#' and each network vertex becomes a 'tip' of the tree. The infection events are
#' labeled with the vertex id of the infector to make it possible to trace the
...
#'
#' param <- param.net.bip(inf.prob = 0.5)
#' init <- init.net.bip(i.num = 1)
#' control <- control.net.bip(type = "SI", nsteps = 40, nsims = 1, verbose = FALSE,
#'                        use.pids = FALSE)
#'
#' mod1 <- netsim.bip(est1, param, init, control)
#' tm <- get_transmat.bip(mod1)
#' tmPhylo <- as.phylo.transmat.bip(tm)
#' plot(tmPhylo, show.node.label = TRUE,
#'               root.edge = TRUE,
#'               cex = 0.5)
#'
as.phylo.transmat.bip <- function(x,
                                  collapse.singles,
                                  vertex.exit.times,
                                  ...) {
  ...
  if (length(v) > 1) {
    ...
    sub_phylos <- lapply(v, function(v_sub) {
      ...
      # call as.phylo on the subset of the edgelist
      as.phylo.transmat.bip(tm[sub_rows, , drop = FALSE],
                            vertex.exit.times = vertex.exit.times)
...
#' @title Converts transmat infection tree into a bipartite network object
#'
#' @description Converts the edges of the infection tree described in the
#'              \code{\link{transmat.bip}} object into a \code{\link{network.bip}} object,
#'              copying in appropriate edge attributes for 'at', 'infDur', 'transProb',
#'              'actRate', and 'finalProb' and constructing a vertex attribute
#'              for 'at'.
#'
...
#' @export
#'
as.network.transmat.bip <- function(x, ...){
  tm <- x
  ids <- unique(c(tm$sus, tm$inf))
...
#' @title Plot transmat infection tree in one of several styles
#'
#' @description Plots the infection tree described in a \code{\link{transmat.bip}}
#'              object in one of several styles: phylogenetic tree, an un-rooted
#'              network, a hierarchical tree, or a transmissionTimeline.
#'
#' @param x A \code{\link{transmat.bip}} object to be plotted
#' @param style Character name of plot style. One of "phylo", "network",
#'        or "transmissionTimeline"
#' @param ...  Additional plot arguments to be passed to lower-level plot
#'        functions (plot.network, plot.phylo, etc)
...
plot.transmat.bip <- function(x,
  ...
  switch(style,
         ...
         "network" = plot.network(as.network(x), ...),
         # "gv_tree" = tm_gv_tree_plot(x, ...),
         "phylo" = plot(as.phylo.transmat.bip(x),
                        show.node.label = TRUE,
                        root.edge = TRUE,
                        label.offset = 0.1,
                        ...
